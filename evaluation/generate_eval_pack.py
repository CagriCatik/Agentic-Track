...
from projects.rag_app.retrieval import extract_keywords
...
def build_pack(*, corpus_dir: Path, max_sources: int) -> dict[str, Any]:
    source_paths = discover_source_files(corpus_dir)
    selected = source_paths[:max_sources]
    loaded = [load_source_document(path, corpus_root=corpus_dir) for path in selected]
